...
- `thread/fullAccess/update` — experimental; enable or disable a runtime-only Full Access override for a loaded thread's live agent tree. Enabling uses the standard `never` approval policy and disabled permission profile for future turns in the root and its current or future subagents. Disabling removes the override and exposes each thread's configured permissions; configured Full Access therefore remains Full Access. Active turns and existing approval requests are unchanged. Returns the resulting `enabled` state and emits `thread/fullAccess/updated`.
...
- `thread/fullAccess/updated` — experimental notification emitted after `thread/fullAccess/update`; includes `threadId` and `enabled`.
