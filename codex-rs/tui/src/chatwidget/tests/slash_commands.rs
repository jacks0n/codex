...
#[tokio::test]
async fn slash_teleport_requests_desktop_handoff() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let thread_id = ThreadId::new();
    chat.thread_id = Some(thread_id);

    chat.dispatch_command(SlashCommand::Teleport);

    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::TeleportToDesktop {
            thread_id: actual_thread_id,
        }) if actual_thread_id == thread_id
    );
}
...
#[tokio::test]
async fn slash_teleport_without_thread_id_shows_starting_error() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;

    chat.dispatch_command(SlashCommand::Teleport);

    let cells = drain_insert_history(&mut rx);
    assert_eq!(cells.len(), 1, "expected teleport startup error");
    assert_eq!(
        lines_to_single_string(&cells[0]),
        "■ Session is still starting; try /teleport again in a moment.\n"
    );
}
